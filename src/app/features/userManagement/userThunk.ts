...
import { InviteUserPayload, InviteUserResponse } from "./userTypes";
...
>("user/inviteUser", async (payload, { rejectWithValue }) => {
  try {
    const response = await userManagementAxiosInstance.post<InviteUserResponse>("api/v1/invite", payload);
...
export const fetchClasses = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await userManagementAxiosInstance.get("/api/v1/class")

      if (response.data.message === "success") {
        return response.data.data // return class list
      } else {
        return rejectWithValue(response.data.message || "Failed to get classes")
      }
    } catch (error) {
...
export const fetchDepartments = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await userManagementAxiosInstance.get('/api/v1/department');
...
export const userDetails = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await userManagementAxiosInstance.get('/api/v1/user-details');
      return response.data.data;
    } catch (err: any) {
      return rejectWithValue(err.response?.data || err.message);
    }
...
>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await userManagementAxiosInstance.get<FetchStudentsResponse>('/api/v1/student');
      return response.data.data;
    } catch (err: any) {
...
>(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      const response = await userManagementAxiosInstance.post('/api/v1/student', payload);
      return response.data;
    } catch (err: any) {
      const message = err.response?.data?.message || err.message || "Failed to add student";
      return rejectWithValue({ error: message });
    }
  }
);
...
>(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const response = await userManagementAxiosInstance.delete(`/api/v1/student/${userId}`);
      return response.data;
    } catch (err: any) {
